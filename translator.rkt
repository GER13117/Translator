...
(define (getPerson ele) ;check nach person
  (cond
    [(query-maybe-value mdbc (string-append "SELECT gender FROM nouns WHERE ele=" "'" ele "'"))(displayln("GESCHLECHT VON NOMEN HERAUSFINDEN"))])
  (displayln ("Noch nicht")))

(define (getVerb person verb)
  (cond
...
(define (transSenWithArticle lst)
  (cond
    ...
    [(query-maybe-value mdbc (string-append "SELECT ger_adj FROM adjectives WHERE eng_adj=" "'" (symbol->string (second lst)) "'")) ;DAS GEHT SO NOCH NICHT
